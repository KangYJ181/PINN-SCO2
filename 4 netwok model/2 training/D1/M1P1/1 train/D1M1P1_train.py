...
def train_and_evaluate(w_loss_weight1, w_loss_weight2, learning_rate, weight_decay, hidden_size):
    ...
    for train_index, valid_index in kf.split(dataset):
        ...
        model.a.data.fill_(0.5463)
        model.b.data.fill_(0.0587)
        model.c.data.fill_(2.07)
        model.d.data.fill_(-0.939)
        model.e.data.fill_(0.579)
        model.f.data.fill_(0.024)
        ...
        for epoch in range(num_epochs):
            ...
            for i, (batch_x, batch_y) in enumerate(train_loader):
                ...
                with torch.no_grad():
                    model.a.data.clamp_(min=0.5, max=0.55)
                    model.b.data.clamp_(min=0.03, max=0.06)
                    model.c.data.clamp_(min=1.5, max=2.1)
                    model.d.data.clamp_(min=-1.0, max=-0.5)
...
pbounds = {'w_loss_weight1': (0.01, 0.49), 'w_loss_weight2': (0.01, 0.49), 'learning_rate': (0.0005,0.001), 'weight_decay': (1e-5,1e-4), 'hidden_size': (8,13)}

# Load optimizer state if it exists
if os.path.exists('optimizer_state_D1M1P1.pkl'):
    with open('optimizer_state_D1M1P1.pkl', 'rb') as f:
...
with open('optimizer_state_D1M1P1.pkl', 'wb') as f:
...
results_df.to_csv('optimization_results_D1M1P1.csv', index=False)