...
w_loss_weight1 = 0.01
w_loss_weight2 = 0.488410732870978
...
final_model.a.data.fill_(0.4887)
final_model.b.data.fill_(1.07148)
final_model.c.data.fill_(2.07908)
...
for epoch in range(num_epochs):
    ...
    for i, (batch_x, batch_y) in enumerate(train_loader):
        ...
        with torch.no_grad():
            final_model.a.data.clamp_(min=0.25, max=0.75)
            final_model.b.data.clamp_(min=0.5, max=1.5)
            final_model.c.data.clamp_(min=1.0, max=3.0)
    ...
    loss_df.to_csv('training_losses_D1M1P2.csv', index=False)
...
# Save model
torch.save(final_model.state_dict(), 'best M1P2 model_D1.pth')
...
coefficients_df.to_csv('optimized_coefficients_D1M1P2.csv', index=False)
...
with torch.no_grad():
    ...
    results_df.to_csv('final_predictions_D1M1P2.csv', index=False)
    ...
    upper_bound = error_line * 1.2
    lower_bound = error_line * 0.8
