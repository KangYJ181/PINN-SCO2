...
start_time = time.time()  # Record the run time
...
def custom_loss(y_pred, batch_y):
    w_loss = nn.MSELoss()(batch_y, y_pred)  # Mean square loss function
    return w_loss
...
data = pd.read_csv('experimental data D2M2P1.csv')  # Load the data
...
pbounds = {'learning_rate': (0.0005, 0.001), 'weight_decay': (1e-5, 1e-4), 'hidden_size':(7,12)}
# Load optimizer state if it exists
if os.path.exists('optimizer_state_D2M2P1.pkl'):
    with open('optimizer_state_D2M2P1.pkl', 'rb') as f:
...
with open('optimizer_state_D2M2P1.pkl', 'wb') as f:
...
results_df.to_csv('optimization_results_D2M2P1.csv', index=False)
